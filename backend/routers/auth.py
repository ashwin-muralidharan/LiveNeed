...
# ---------------------------------------------------------------------------
# Super-admin: list & delete admins
# ---------------------------------------------------------------------------
SUPER_ADMIN_EMAILS = {"maneesh@gmail.com", "ashwin@gmail.com"}


@router.get("/admins")
def list_admins(admin: AdminUser = Depends(get_current_admin), db: Session = Depends(get_db)):
    """List all approved admin users."""
    admins = db.query(AdminUser).filter(AdminUser.is_approved == True).order_by(AdminUser.created_at.asc()).all()
    return [
        {
            "id": a.id,
            "name": a.name,
            "email": a.email,
            "is_super": a.email in SUPER_ADMIN_EMAILS,
...
def delete_admin(admin_id: int, admin: AdminUser = Depends(get_current_admin), db: Session = Depends(get_db)):
    """Delete an admin user. Only super admins (maneesh@gmail.com, ashwin@gmail.com) can do this."""
    if admin.email not in SUPER_ADMIN_EMAILS:
    ...
    if target.email in SUPER_ADMIN_EMAILS:
        raise HTTPException(status_code=403, detail="Cannot delete a super admin account")

    db.delete(target)
    db.commit()
    return {"message": f"Admin '{target.name}' ({target.email}) has been removed"}
